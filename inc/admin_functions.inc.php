...
function hesk_deleteTicketsForCustomer($customer_id, $solo_only = true) {
    global $hesk_settings;

    $solo_sql =" AND `tickets`.`id` IN (
        SELECT `ticket_to_customer_inner`.`ticket_id`
        FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."ticket_to_customer` AS `ticket_to_customer_inner`
        GROUP BY `ticket_to_customer_inner`.`ticket_id`
        HAVING COUNT(`ticket_to_customer_inner`.`ticket_id`) = 1
    )";
